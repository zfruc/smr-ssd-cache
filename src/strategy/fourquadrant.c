...
{
	unsigned long	band_num = GetSMRZoneNumFromSSD(new_ssd_buf_hdr->ssd_buf_tag.offset);
	if (band_descriptors_for_maxcold_evict_now[band_num].ischosen_tocache == 0) {
        return NULL;
    }
...
bool
isCachedMaxColdEvict(SSDBufferDesc * ssd_buf_hdr)
{
	unsigned long	band_num = GetSMRZoneNumFromSSD(ssd_buf_hdr->ssd_buf_tag.offset);
    return band_descriptors_for_maxcold_evict_now[band_num].ischosen_tocached;
}
