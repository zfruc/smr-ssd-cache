...
initSSD()
{
    ...
    initSSDBufTable(NTABLE_SSD_CACHE);

    ssd_buffer_strategy_control = (SSDBufferStrategyControl *)SHMalloc(sizeof(SSDBufferStrategyControl));
    ssd_buffer_strategy_control->n_usedssd = 0;
    ssd_buffer_strategy_control->first_freessd = 0;
    ssd_buffer_strategy_control->last_freessd = NBLOCK_SSD_CACHE - 1;

/*    ssd_buffer_descriptors = (SSDBufferDesc *)SHMalloc(sizeof(SSDBufferDesc) * NBLOCK_SSD_CACHE);*/
    ...
    ssd_buffer_descriptors[NBLOCK_SSD_CACHE - 1].next_freessd = -1;
    hit_num = 0;
    flush_ssd_blocks = 0;
    read_ssd_blocks = 0;
    time_read_cmr = 0.0;
...
read_block(off_t offset, char *ssd_buffer)
{
    ...
    {
        returnCode = smrread(hdd_fd, ssd_buffer, SSD_BUFFER_SIZE, ssd_buf_tag.offset);
        if (returnCode < 0)
        {
    ...
    else //LRU ...
    {
        ...
        {
            returnCode = smrread(hdd_fd, ssd_buffer, SSD_BUFFER_SIZE, offset);
